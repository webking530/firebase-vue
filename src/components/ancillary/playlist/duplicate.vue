      ...
      "
        >
          ...
          <b-form @submit.stop.prevent="Submit" class="mb-3 pb-3">
            ...
            >
              <b-button
                @click="$bvModal.hide('duplicate-playlist-modal')"
...
export default {
  ...
  methods: {
    ...mapActions("Playlist", ["duplicate_playlist"]),
    validateState(name) {
    ...
    Submit() {
      ...
      this.duplicate_playlist([this.id, this.form])
        .then((res) => {
          res;
          ...
          setTimeout(() => {
            this.$bvModal.hide("duplicate-playlist-modal");
            this.$router.push({
              name: "Playlist",
