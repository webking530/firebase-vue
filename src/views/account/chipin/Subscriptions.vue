<template>
  <div>
    ...
    <b-container v-if="loaded" class="mb-5">
        ...
        <SettingsTitle v-bind:title="'Subscriptions'"></SettingsTitle>
        
        <b-row class="mt-3">
          <SectionHeading v-bind:name="'Subscriptions'"></SectionHeading>
...
export default {
  ...
  methods: {
    ...mapActions("Chipinmanager", { api_subscriptions: "subscriptions" }),
    cancelSubscription(sub_id, index) {
      
      axios
        .put("/chipin/account/" + sub_id + "/subscriptions/cancel/")
        .then((res) => {
          res
